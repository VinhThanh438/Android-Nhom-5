...
import nhom5.android_n5_manager_account.Model.ExpensesObject;
import nhom5.android_n5_manager_account.Model.PayingTuitionObject;
import nhom5.android_n5_manager_account.Model.PlanObject;
import nhom5.android_n5_manager_account.Model.SubjectObject;
import nhom5.android_n5_manager_account.Model.UserObject;
...
public List<ExpensesObject> getExpensesObjectOfYear(int userId,String year) {
    ...
    public void insertSubject(String subjectName, int studyCredits, int semester) {
        SQLiteDatabase db = this.getWritableDatabase();
        ContentValues values = new ContentValues();
        ...
        if (result == -1) {
            Log.e("Database Error", "Failed to insert subject: " + subjectName);
        } else {
            Log.d("Database Success", "Subject inserted with ID: " + result);
        }

        db.close(); // Đóng cơ sở dữ liệu sau khi chèn
    }
